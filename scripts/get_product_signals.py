...
BAD_TITLE_WORDS = [
    ...
    "转会", "球员", "伤病", "比分", "赛事直播",
    "超值好货", "省钱快报", "加拿大省钱快报",
    "t.cn", "http://t.cn", "微博", "超话", "抽奖", "转发本条", "我来了",
    "偶遇", "哈哈", "打call", "送花花", "🔥", "👀", "🎁", "💯"
]

BAD_SOURCES = [
    "雪球", "东方财富", "新浪财经", "证券时报", "财联社", "华尔街见闻",
    "加拿大省钱快报", "北美省钱快报", "微博", "手机新浪网"
]
...
def title_has_good_product_signal(title):
    return any(w in title for w in GOOD_PRODUCT_WORDS)
...
def should_drop_item(title, source, pub_date):
    ...
    if " - 新浪网" in title and not title_has_good_product_signal(title):
        return True
...
def detect_brand(title):
    """
    只从标题识别品牌，不从query继承。
    这样可以避免“query=安踏儿童 防晒衣，但标题是京东服饰春尚新”也被误判为安踏儿童。
    """
    hits = []
...
def score_signal(title, query, brands, keywords, source):
    ...
    trusted_sources = [
        "美通社", "36氪", "新华网", "人民网", "澎湃", "界面", "中国日报",
        "新京报", "搜狐", "凤凰网", "腾讯", "中华网", "赢商网", "亿邦动力",
    ...
    if any(w in query for w in SHOE_TECH_KEYWORDS):
        score += 5
    if any(s in source or s in title for s in trusted_sources):
        score += 5
...
summary = {
    ...
    "desc": "全年运动鞋服商品趋势信号监测：覆盖儿童、成人、童装、四季商品、大促节点、鞋科技、运动场景。已过滤过旧新闻、财经财报、微博广告和弱相关信号。该数据代表公开资讯热度信号，不等同于平台真实销量。",
