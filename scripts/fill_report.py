...
focus_type：4-18字，经营抓手，可以从“商品机会、会员运营、商圈活动、直播承接、价格带经营、校园场景、亲子场景、户外场景、赛事营销、内容种草”中选择；
...
6. 每个区域必须优先围绕该区域 theme 展开，但 theme 只能来自区域新闻自动识别，不得强行固定。
7. 不同区域可以出现相同 theme，比如今天华东和华南都可以是足球运动，只要各自区域新闻支持。
8. 同一区域如果连续几天新闻主题相同，也允许重复，但表达角度必须变化。
9. focus_type必须根据该区域当天新闻、天气和theme自动判断，不允许固定区域对应抓手；
10. 不同区域可以出现相同focus_type，但不能所有区域完全一样；
11. action必须是短动作，例如：成长鞋专区、亲子会员日、抖音同款区、防晒单品组合、银发家庭组合购。
...
"""
    ...
    for region in region_map.keys():
        ...
        focus_type = short_cn(
            row.get('focus_type', '商品机会'),
            12
        )

        action = short_cn(
            row.get('action', fallback_actions[region]),
            20
        )
