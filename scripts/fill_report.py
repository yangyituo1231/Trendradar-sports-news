...
THEME_KEYWORDS = {
    '足球运动':['足球','足弓','球鞋','训练','青训','实验室'],
    '赛事经济':['赛事','比赛','马拉松','运动会','体育节'],
...
def detect_region_theme(local_news):
    ...
    for theme, keywords in THEME_KEYWORDS.items():
        scores[theme] = sum(10 for kw in keywords if kw in text)
...
def build_region_reports_rule():
    ...
    for region, cfg in region_map.items():
        ...
        reports[region] = {
            'change': f"{theme}带动区域消费变化",
            'impact': f"{cfg['city']}客流与消费场景受新闻和天气共同影响",
            'focus': '商品机会'
        }
...
hot：18-26字，核心信号；
flow：24-42字，客流场景变化；
focus_type：4-12字，经营抓手；
action：18-36字，动作建议。

要求：
1. 必须优先使用区域数据里的news_titles、weather、theme。
2. focus_type和action都必须根据区域当天新闻、天气、theme自动判断，不允许固定区域对应抓手。
3. action必须是经营动作，不要只写品类名称。
4. action要结合实际情况，可以包含商品组合、陈列专区、会员触达、导购话术、直播同款、亲子活动、商圈联动、价格带、天气品类、防滑防雨、防晒凉感等。
5. action不要太短，不要只写“亲子会员日”“防晒单品组合”。
6. action不要以“建议：”开头。
7. 五个区域的action不能完全重复。
8. 不要编造不存在的区域城市事件。
9. 允许同一区域同时体现新闻和天气，例如“防晒凉感陈列+亲子试穿引导”。
...
"""
    ...
    for region in region_map.keys():
        ...
        if action in actions.values():
            action = short_cn(f"{focus_type}差异化陈列与会员触达", 30)
...
def make_ai_warnings():
    ...
    weather_text = '；'.join([weather_desc(k) for k in ['north','east','south','southwest','northwest']])
    prompt = f"""
你是361°儿童经营管理负责人。请基于今日行业新闻、全国天气、电商大促、区域消费趋势、重大品牌事件、宏观消费、政策、社零/GDP、文旅出行、AI科技热点，生成3条每日动态经营关注点，必须随当天新闻变化。。
要求：输出严格JSON数组；每条25-50字；像总部经营预警；不要空话；必须具体到客流、品类、会员、直播、天气、商圈、区域、价格带、品牌声量或政策影响。必须是完整句子，不允许输出半句。如果TOP资讯中出现品牌签约、代言、联名、发布会、爆火、出圈等事件，必须至少有1条关注点围绕该事件的经营启示。
新闻：
{news_text}
天气：
{weather_text}
"""
    ...
    for x in arr:
        ...
        fallback_pool = [
            '重点品牌事件升温，需关注搜索热度、竞品动作和终端话术承接。',
            '平台大促与直播流量波动，需关注核心SKU库存、价格带和同款转化。',
            '天气变化影响到店节奏，需关注防晒、防雨、防滑和室内运动场景。',
            '儿童运动与亲子场景升温，门店需强化会员触达和试穿转化。',
            '商圈活动与区域客流分化，重点门店需动态调整陈列和导购重点。',
            '宏观消费预期仍有分化，需关注高性价比商品、折扣效率和客单变化。',
            'AI与内容平台热点升温，需关注小红书、抖音种草和商品卖点表达。'
        ]

        while len(result) < 3:
           result.append(fallback_pool[len(result)])
...
for region in ['east','central','south','southwest','northwest']:
    ...
    action_text = clean_title(action_text)
