...
7. 语气像总部给区域销售看的经营提醒；
8. 内容适合放进日报表格。
9. 同一区域今天可能是商圈，明天可能是天气，后天可能是校园或文旅，判断方向必须随输入数据变化。
10. 每个区域必须说明“为什么今天关注这个点”，触发依据必须来自区域新闻、天气或全国热点。
11. 五个区域表达要有差异，但差异来自数据，不来自人为固定分工。
12. action不能重复，必须根据该区域当日触发因素生成不同动作。
输出严格JSON对象，不要解释。
