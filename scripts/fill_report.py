...
12. 如果 news_titles 不为空，hot、flow、signal 三项中至少两项必须直接引用 news_titles 中的具体事件或地点；
13. 不允许使用“线上内容种草可能外溢到门店试穿和比价”这种通用句；
14. 每个区域必须写出一个不同的触发来源，例如：本地商圈、区域天气、平台大促、文旅客流、AI科技、品牌动作；
15. 如果五个区域出现相同 signal 或 action，视为错误，必须重写为不同内容。
