...
def build_region_reports_rule():
    ...
    for region, cfg in region_map.items():
        ...
        focus_type = detect_region_theme(local_news)

        reports[region] = {
            'change': pick_scene_word(scenes, 'hot'),
            'impact': pick_scene_word(scenes, 'flow'),
            'focus': short_cn(focus_type, 12)
        }

        actions[region] = short_cn(
            pick_scene_word(scenes, 'action').replace('建议：', ''),
            20
        )
...
focus_type：4-18字，经营抓手，可从商品机会、会员运营、商圈活动、直播承接、价格带经营、校园场景、亲子场景、户外场景、赛事营销、内容种草中选择，也可根据新闻自拟；
action：8-18字，具体动作建议，不要写长句，不要以“建议：”开头。

要求：
1. 必须优先使用区域数据里的news_titles；
2. hot、flow要尽量体现该区域城市、商圈、活动或事件；
3. 不得使用通用模板句；
4. 五个区域的hot、focus_type、action不能完全重复；
5. focus_type必须根据该区域当天新闻、天气和theme自动判断，不允许固定区域对应抓手；
6. 不同区域可以出现相同focus_type，但不能所有区域完全一样；
7. action必须是短动作，例如：成长鞋专区、亲子会员日、抖音同款区、防晒单品组合、银发家庭组合购。
...
"""
    ...
    for region in region_map.keys():
        ...
        action = short_cn(action, 20)

        if not action:
            action = fallback_actions[region]

        if action in actions.values():
            action = short_cn(f"{focus_type}专项陈列", 20)
...
for region in ['east','central','south','southwest','northwest']:
    ...
    data[f'{region}_hot'] = reports[region].get(
        'change',
        reports[region].get('hot','')
    )

    data[f'{region}_flow'] = reports[region].get(
        'impact',
        reports[region].get('flow','')
    )

    data[f'{region}_focus'] = reports[region].get(
        'focus',
        reports[region].get(
            'focus_type',
            reports[region].get('signal','')
        )
    )
