...
signal：38-55字，写成“触发因素 -> 经营影响 -> 重点品类/场景”；
action：38-58字，必须以“建议：”开头，至少包含2个具体动作。
...
"""
    ...
    for region in region_map.keys():
        ...
        signal = short_cn(row.get('signal', fallback_reports[region]['action']), 68)
        action = short_cn(row.get('action', fallback_actions[region]), 72)

        action = re.sub(r'^建议[:：\s]*', '', action)
        action = re.sub(r'^[0-9]+\.\s*', '', action)
        action = short_cn(action, 72)
