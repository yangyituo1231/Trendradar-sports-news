...
要求：输出严格JSON数组；每条20-30字；像总部经营预警；不要空话；必须具体到客流、品类、会员、直播、天气、商圈、区域、价格带或政策影响。
新闻：
{news_text}
天气：
{weather_text}
"""
    ...
    result = [clean_title(str(x)).replace('【预警：','').replace('】','')[:34] for x in arr if clean_title(str(x))]
