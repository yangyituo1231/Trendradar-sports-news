...
每个区域包含：hot：核心信号，14-22字；flow：客流/消费场景判断，24-38字；signal：区域经营判断，40-60字；action：动作建议，40-60字。
今日TOP资讯：
{top_news_text}
全国新闻：
{global_news_text}
...
def build_words_deepseek():
    ...
    weather_text = '；'.join([weather_desc(k) for k in ['north','east','south','southwest','northwest']])
    prompt = f"""
你是运动鞋服行业情报系统。请基于今日行业新闻、TOP重点资讯、全国天气、当前消费趋势、宏观消费、GDP/社零、就业收入、文旅出行、AI科技、社会热点，生成18个适合“行业热词雷达”的真实热词。
要求：输出严格JSON数组；每个词2-8字；更像当天行业热点；覆盖鞋服、童装、户外跑步、电商平台、品牌竞争、天气消费、AI科技、年轻人、体育精神、国际化、出海、城市消费、健康生活、社会热点、情绪消费、校园体育、智能硬件、多品牌、宏观消费。
TOP资讯：
{top_text}
新闻：
{news_text}
